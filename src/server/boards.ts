...
function getGeminiClient() {
  ...
  if (!key || key === "MY_GEMINI_API_KEY") {
    throw new Error("GEMINI_API_KEY is not configured in environment. Please add it to your platform secrets in the Settings panel.");
  }
  if (!aiClient) {
...
function optionalAuthenticateToken(req: any, res: Response, next: () => void) {
  ...
  try {
    // Require standard inline library call or direct import if jwt is available
...
boardsRouter.get("/public-stats", async (req: any, res: Response): Promise<void> => {
  try {
    const publicBoards = await db.getBoards({ isPublic: true });
    const decisionCount = publicBoards.filter(b => b.category === "Decision").length;
    const ideaCount = publicBoards.filter(b => b.category === "Idea").length;
    const planCount = publicBoards.filter(b => b.category === "Plan").length;
...
boardsRouter.get("/", optionalAuthenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    ...
    const publicBoards = await db.getBoards({ search, category, isPublic: true });
    
    // Fetch user boards if logged in
    let userBoards: IBoard[] = [];
    if (userId) {
      userBoards = await db.getBoards({ search, category, ownerId: userId, isPublic: false });
    }
...
boardsRouter.get("/summary", authenticateToken as any, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ error: "Failed to generate dashboard summary stats." });
  }
});

// Get single board by ID (with message history and generated outputs)
boardsRouter.get("/:id", optionalAuthenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
...
// Create board
boardsRouter.post("/", authenticateToken as any, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { title, description, context, category } = req.body;
    const userId = req.user?.userId;

    if (!title || !description || !context || !category) {
      res.status(400).json({ error: "Missing required fields: title, description, context, category." });
      return;
    }
...
boardsRouter.post("/:id/clone", authenticateToken as any, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const userId = req.user?.userId;
    ...
    if (!sourceBoard.isPublic) {
      res.status(403).json({ error: "Can only clone public template boards." });
      return;
    }
...
boardsRouter.put("/:id", authenticateToken as any, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    if (board.ownerId !== userId) {
      res.status(403).json({ error: "Unauthorized. You do not own this board." });
      return;
    }
...
// Delete board
boardsRouter.delete("/:id", authenticateToken as any, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const userId = req.user?.userId;
...
boardsRouter.post("/:id/chat", authenticateToken as any, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    if (board.ownerId !== userId) {
      res.status(403).json({ error: "Unauthorized access to private board." });
      return;
    }
...
4. IMPORTANT: You must return your response in JSON format matching the schema provided. The response fields are:
   ...
   - "suggestions": An array of exactly 3 relevant, click-worthy follow-up questions or prompts that help the user think deeper or proceed.`;
        ...
        responseSchema: {
          ...
          properties: {
            ...
            suggestions: {
              type: "array",
              items: { type: "string" },
              description: "Exactly three suggested follow-up actions/questions.",
            },
          },
    ...
    res.status(500).json({ error: err.message || "Failed to generate AI chat response." });
  }
});

// AI Content Generator Endpoint (Strictly pure Gemini API, throwing errors if missing)
boardsRouter.post("/:id/generate", authenticateToken as any, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    if (!type || !length) {
      res.status(400).json({ error: "Content type and length parameters are required." });
      return;
    }
    ...
    const historyText = history.map(m => `${m.role === "user" ? "User" : "Aivora"}: ${m.content}`).join("\n\n");
...
Begin generating immediately in markdown format:`;
    ...
    const specificTypeOutputs = existingOutputs.filter(o => o.type === type);
    const nextVersion = specificTypeOutputs.length > 0 ? Math.max(...specificTypeOutputs.map(o => o.version)) + 1 : 1;
    ...
    res.status(500).json({ error: err.message || "Failed to generate document content." });
  }
});

// AI Smart Recommendations Endpoint (Strictly pure Gemini API, throwing errors if missing)
boardsRouter.get("/:id/recommendations", authenticateToken as any, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const userId = req.user?.userId;
    ...
    const historyText = history.slice(-5).map(m => `${m.role === "user" ? "User" : "Aivora"}: ${m.content}`).join("\n");
...
The output MUST be in JSON format matching the schema:
{
  "recommendations": [
            ...
            recommendations: {
              ...
              items: {
                ...
                properties: {
                  ...
                  actionType: { type: "string", enum: ["chat", "generate", "research"] },
                },
    ...
    res.status(500).json({ error: err.message || "Failed to load smart recommendations." });
  }
});
